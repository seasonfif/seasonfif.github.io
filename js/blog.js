...
(function (w, d) {
    ...
    var Blog = {
        ...
        toc: (function () {
            ...
            var bannerH = $('#header_top').clientHeight,
                headerH = $('#header').clientHeight,
                titles = $('#post-content').querySelectorAll('h1, h2, h3, h4, h5, h6');

            if (toc && titles.length) {
                toc.querySelector('a[href="#' + titles[0].id + '"]').parentNode.classList.add('active');
            }
            
            return {
                fixed: function (top) {
                    if(top > 0 && headerH - top > 0){
                        toc.setAttribute("style", "margin-top:" + (150 - top) + 'px');
                    }
                    
                    if(top - headerH >= 0){
                        toc.classList.add('fixed');
                        toc.classList.remove('top')
                        toc.setAttribute("style", "margin-top:" + 0 + 'px');
                    }else{
